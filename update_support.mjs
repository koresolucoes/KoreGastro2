...
export class SupportClientComponent implements OnInit {
  ...
  async createNewTicket() {
    ...
    try {
      ...
      if (error.message && error.message.includes('row-level security')) {
         this.notificationService.alert('Aviso de Segurança (RLS): O seu banco de dados precisa que as políticas de segurança da tabela "support_tickets" sejam configuradas. Por favor, acesse o painel do Supabase e rode o script SQL mais recente.', 10000);
      } else {
  ...
  async markAsResolved(ticketId: string) {
      ...
      try {
          ...
          console.error(error);
          if (error.message && error.message.includes('row-level security')) {
             this.notificationService.alert('Erro de Permissão. O banco de dados precisa ser atualizado com as políticas corretas no Supabase.', 10000);
          } else {
