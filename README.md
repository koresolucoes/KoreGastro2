...
A API de RH permite a integração com aplicativos externos de controle de ponto, como um tablet na entrada do restaurante.

A autenticação segue o mesmo padrão das outras APIs, usando uma chave Bearer.

**Header:** `Authorization: Bearer SUA_CHAVE_DE_API_EXTERNA`
---
#### `GET /api/rh`

Use este endpoint para buscar a lista de funcionários ativos.

**Query Parameters:**

*   `restaurantId` (obrigatório): O ID do seu usuário no sistema ChefOS.
*   `action` (obrigatório): Deve ser `employees`.

**Exemplo de Requisição:**
```
GET https://gastro.koresolucoes.com.br/api/rh?restaurantId=SEU_USER_ID&action=employees
Authorization: Bearer SUA_CHAVE_DE_API_EXTERNA
```
...
Use este endpoint para registrar ações de ponto para um funcionário.
...
**Campos:**
*   `restaurantId` (obrigatório): String.
*   `employeeId` (obrigatório): String. O UUID do funcionário.
*   `pin` (obrigatório): String. O PIN de 4 dígitos do funcionário.
*   `action` (obrigatório): String. A ação a ser executada. Valores possíveis:
    *   `"clock_in"`: Registra a entrada do funcionário.
    *   `"clock_out"`: Registra a saída do funcionário.
    *   `"start_break"`: Registra o início de uma pausa.
    *   `"end_break"`: Registra o fim de uma pausa.

**Resposta (Sucesso 200 OK):**
```json
{
  "success": true,
  "message": "Clock-in successful for Ana Gerente."
}
```

**Respostas de Erro:**

*   **400 Bad Request:** Campos faltando ou `action` inválida.
*   **401 Unauthorized:** O `pin` fornecido está incorreto.
*   **404 Not Found:** O `employeeId` não foi encontrado.
*   **409 Conflict:** A ação não é permitida no estado atual (ex: `clock_in` quando já está em um turno).
