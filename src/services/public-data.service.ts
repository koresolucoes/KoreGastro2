...
export class PublicDataService {
  async getPublicVirtualMenu(
    userId: string,
  ): Promise<{
    recipes: (Recipe & { menu_item_id?: string })[];
    categories: Category[];
    optionGroups: IfoodOptionGroup[];
    recipeOptionGroups: RecipeIfoodOptionGroup[];
  } | null> {
    ...
    const outOfStockIds = new Set((stockData as any).outOfStockRecipeIds || []);

    for (const recipe of baseRecipes) {
      (recipe as any).hasStock = !outOfStockIds.has(recipe.id);
    }
    ...
    if (onlineMenus.length === 0) {
      return {
        ...
        optionGroups: baseOptionGroups,
        recipeOptionGroups: baseRecipeOptionGroups,
      };
    }

    const validMenuIds = new Set(onlineMenus.map((m) => m.id));
    const activeMenuCategories = (menuCategories || []).filter((c) =>
      validMenuIds.has(c.menu_id),
    );
    const validCatIds = new Set(activeMenuCategories.map((c) => c.id));

    // Virtual Categories
    const virtualCategories = activeMenuCategories
    ...
    const activeMenuItems = (menuItems || [])
      .filter((i) => validCatIds.has(i.menu_category_id))
      .sort((a, b) => (a.display_order ?? 0) - (b.display_order ?? 0));
    const recipesMap = new Map(baseRecipes.map((r) => [r.id, r]));

    // Virtual Recipes
    const virtualRecipes = activeMenuItems
      .map((item) => {
    ...
    for (const virtualR of virtualRecipes) {
      ...
      const opts = (menuOptions || []).filter(
        (o) => o.menu_item_id === virtualR.menu_item_id,
      );

      for (const opt of opts) {
        const choices = (menuChoices || []).filter(
          (c) => c.menu_item_option_id === opt.id,
        );
        const virtualChoices = choices
          .map((choice) => {
            const linkedRecipe = choice.recipe_id
              ? recipesMap.get(choice.recipe_id)
              : null;
            return {
              id: choice.id,
              user_id: opt.user_id,
              ...
              hasStock: linkedRecipe
                ? (linkedRecipe as any).hasStock !== false
                : true,
            };
          })
  ...
  async getPublicLoyaltySettings(
    userId: string,
  ...
  async getPublicRecipeOptionGroups(
    userId: string,
  ): Promise<RecipeIfoodOptionGroup[]> {
    const { data, error } = await supabase
      .from("recipe_ifood_option_groups")
      .select("*")
      .eq("user_id", userId);
