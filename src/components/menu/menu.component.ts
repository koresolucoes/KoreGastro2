...
export class MenuComponent implements OnInit {
  ...
  async ngOnInit() {
    this.route.params.subscribe(async (params) => {
      ...
      if (id) {
        ...
        console.error(
          "Menu Online: Nenhum ID de loja encontrado. Redirecionando para home.",
        );
  ...
  menuGroups = computed(() => {
    ...
    return categories
      .map((cat) => {
        ...
        const filtered = catRecipes.filter(
          (r) =>
  ...
  isRestaurantOpen = computed(() => {
    ...
    return (
      currentStr >= schedule.opening_time && currentStr <= schedule.closing_time
    );
  });
  ...
  async onConfirmTableOrder() {
    ...
    try {
        ...
        if (item.options && item.options.length > 0) {
          ...
          groups.forEach((g) => {
            ...
            if (selectedInGroup && selectedInGroup.length > 0) {
              groupStrings.push(
                `${g.name}: ${selectedInGroup.map((o) => o.name).join(", ")}`,
              );
              finalCostOptions += selectedInGroup.reduce(
                (sum, o) => sum + o.price,
                0,
              );
            }
      ...
      if (!response.ok)
        throw new Error(resData.error || "Erro ao inserir itens no pedido");
  ...
  async callWaiter() {
    ...
    try {
      ...
      this.notificationService.show(
        "Garçom chamado! Em breve alguém irá te atender.",
        "success",
      );
    } catch (e: any) {
      console.error(e);
  ...
  async requestBill() {
    ...
    try {
      try {
        await fetch(`/api/public-request-bill?token=${this.sessionToken()}`);
      } catch (e) {}
      ...
      this.view.set("menu");
      this.notificationService.show(
        "Fechamento de conta solicitado ao caixa.",
        "success",
      );
    } catch (e: any) {
      console.error(e);
  ...
  async onConfirmOrder(event: {
    ...
    try {
      ...
      const orderData = {
        ...
        order_type:
          event.type === "delivery" ? "External-Delivery" : "QuickSale",
        status: "OPEN",
        notes: `Contato: ${event.phone}${event.address ? " | Endereço: " + event.address : ""}`,
        delivery_info:
          event.type === "delivery" ? { address: event.address } : null,
      };
      ...
      const resData = await response.json();
      if (!response.ok)
        throw new Error(resData.error || "Erro ao processar o pedido online");
