...
export class PaymentModalComponent {
  ...
  orderSubtotalBeforeDiscount = computed(() => this.lastKnownOrder()?.order_items.filter((i: any) => !(i.notes?.includes('[AUX_PREP_IDX:') && !i.notes?.includes('[AUX_PREP_IDX:0]'))).reduce((sum: number, item: any) => Math.round(sum * 100 + item.price * 100 * item.quantity) / 100, 0) ?? 0);
