...
export class PublicTableOrderComponent implements OnInit, OnDestroy {
  ...
  companyProfile = signal<any>(null);
  ...
  async ngOnInit() {
    ...
    // Polling as fallback for realtime since RLS blocks anonymous realtime events
    this.pollingInterval = setInterval(() => {
      if (!this.showCheckoutModal() && !this.loading()) {
        this.loadOrder(true);
      }
    }, 10000);
  }
  ...
  async loadOrder(isPolling = false) {
    if (!isPolling) this.loading.set(true);
    const { order, error } =
      await this.publicDataService.getOrderBySessionToken(this.sessionToken()!);

    // If order is missing, or status is completed/cancelled, it's considered closed
    if (
      error ||
      !order ||
      ...
      this.errorMsg.set(
        "Comanda fechada ou inválida. Redirecionando para o cardápio...",
      );
      if (this.pollingInterval) clearInterval(this.pollingInterval);

      // Attempt to redirect if we have user_id, otherwise go to home
      setTimeout(() => {
        if (order && order.user_id) {
          this.router.navigate(["/cardapio", order.user_id]);
        } else {
      ...
      if (!this.companyProfile()) {
        const profile = await this.publicDataService.getPublicCompanyProfile(
          order.user_id,
        );
  ...
  confirmAddToCart() {
    ...
    this.cartItems.update((items) => {
      ...
      const existing = extData.find(
        (i) => i.recipe.id === recipe.id && i.notes === note,
      );
      if (existing) {
        existing.quantity++;
      } else {
  ...
  get cartTotalAmount(): number {
    return this.cartItems().reduce(
      (acc, item) => acc + Number(item.recipe.price || 0) * item.quantity,
      0,
    );
  }
  ...
  async submitCart() {
    ...
    if (!response.ok) {
      this.errorMsg.set(
        "Erro ao enviar pedido: " + (resData.error || "Falha no servidor"),
      );
  ...
  private showToast(message: string, isError = false) {
    ...
    toastDiv.className = `fixed top-4 left-1/2 -translate-x-1/2 z-[9999] px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 font-bold animate-in slide-in-from-top fade-in duration-300 ${isError ? "bg-red-500 text-white" : "bg-zinc-900 text-white"}`;
    toastDiv.innerHTML = `<span class="material-symbols-outlined text-xl">${isError ? "error" : "check_circle"}</span> <span>${message}</span>`;
    document.body.appendChild(toastDiv);
    setTimeout(() => {
      toastDiv.classList.replace("animate-in", "animate-out");
      toastDiv.classList.replace("fade-in", "fade-out");
      toastDiv.classList.replace("slide-in-from-top", "slide-out-to-top");
  ...
  async onCallWaiter() {
    ...
    const { error: rpcError } = await supabase.rpc("public_call_waiter", {
  ...
  async confirmCheckout() {
    ...
    if (this.checkoutMethod() === "PIX") {
      this.showToast("Aguardando confirmação do Mercado Pago...");

      // After some seconds or if they click, we pretend it's paid for UX
      setTimeout(async () => {
        const amount = this.splitTotalPerPerson();
