...
export default withAuth(async function handler(req: any, res: any, restaurantId: string) {
    ...
    if (req.method === 'GET') {
        const listCacheKey = `recipe-categories:${restaurantId}`;
        const fetchList = async () => {
            const { data, error } = await supabase.from('categories').select('*').eq('user_id', restaurantId).order('name', { ascending: true });
    ...
    if (req.method === 'POST') {
        ...
        await invalidateCachePattern(`recipe-categories:${restaurantId}`);
    ...
    if (req.method === 'PUT') {
        ...
        const { data, error } = await supabase.from('categories').update(payload).eq('id', id).eq('user_id', restaurantId).select().single();
        if (error) throw error;
    ...
    if (req.method === 'DELETE') {
        ...
        const { error } = await supabase.from('categories').delete().eq('id', id).eq('user_id', restaurantId);
        if (error) throw error;
