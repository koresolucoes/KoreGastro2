...
import { remember, invalidateCachePattern, deleteCache } from '../utils/redis.js';
import { z } from 'zod';
...
async function handleGet(req: VercelRequest, res: VercelResponse, restaurantId: string) {
  const { tableId, nocache } = req.query;

  if (tableId && typeof tableId === 'string') {
    const cacheKey = `table:${restaurantId}:${tableId}`;
    const fetcher = async () => {
    ...
    const item = nocache === 'true' ? await fetcher() : await remember(cacheKey, 180, fetcher);
    if (!item) {
      return res.status(404).json({ type: "about:blank", title: "Not Found", status: 404, detail: `Table with id "${tableId}" not found.` });
    }
    return res.status(200).json(item);
  }

  const hallId = req.query.hallId as string || 'all';
  const status = req.query.status as string || 'all';
  const limit = parseInt(req.query.limit as string) || 50;
  const cursor = (req.query.cursor as string) || 'none';

  const listCacheKey = `tables:${restaurantId}:${hallId}:${status}:${limit}:${cursor}`;
  ...
  const fetchList = async () => {
    ...
    if (req.query.cursor) query = query.gt('number', req.query.cursor as string);
  ...
  const tables = nocache === 'true' ? await fetchList() : await remember(listCacheKey, 120, fetchList);
...
async function handlePost(req: VercelRequest, res: VercelResponse, restaurantId: string) {
  ...
  await invalidateCachePattern(`tables:${restaurantId}:*`);
...
async function handlePatch(req: VercelRequest, res: VercelResponse, restaurantId: string) {
  ...
  await Promise.all([
    deleteCache(`table:${restaurantId}:${tableId}`),
